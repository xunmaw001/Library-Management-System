<template>
    <div class="main-content">
        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '借阅状态' : ''">
                        <el-select v-model="searchForm.shujiYuyueTypes" placeholder="请选择借阅状态">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in shujiYuyueTypesSelectSearch"
                               v-bind:key="index"
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           ...
                           href="http://localhost:8080/tushuguanlixitong/upload/shujiYuyueMuBan.xls"
                        >批量导入书籍借阅数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('shujiYuyue','导入导出')"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      ...
                                      label="借阅天数">
                        <template slot-scope="scope">
                            {{scope.row.shujiYuyueNum}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="shujiYuyueTypes"
                                      header-align="center"
                                      label="借阅状态">
                        <template slot-scope="scope">
                            {{scope.row.shujiYuyueValue}}
                        </template>
                    </el-table-column>
            ...
            <el-button v-if="isAuth('shujiYuyue','订单') && scope.row.shujiYuyueTypes == 101 && sessionTable=='yonghu' && userId==scope.row.yonghuId" type="primary" icon="el-icon-sold-out" size="mini" @click="refund(scope.row.id)">提前还书</el-button>
            <el-button v-if="isAuth('shujiYuyue','订单') && scope.row.shujiYuyueTypes == 104 && sessionTable=='yonghu' && userId==scope.row.yonghuId" type="primary" icon="el-icon-reading" size="mini" @click="openCommentModel(scope.row.id)">评价</el-button>
            <el-button v-if="isAuth('shujiYuyue','订单') && scope.row.shujiYuyueTypes == 101
&& sessionTable=='users'" type="primary" icon="el-icon-reading" size="mini"
                        @click="deliver(scope.row.id)"
            >到期</el-button>
            <el-button v-if="isAuth('shujiYuyue','订单') && scope.row.shujiYuyueTypes == 103 && sessionTable=='yonghu' && userId==scope.row.yonghuId" type="primary" icon="el-icon-reading" size="mini" @click="receiving(scope.row.id)">还书</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
            ...
            form:{
                ...
                shujiYuyueTime : null,
                shujiYuyueNum : null,
                shujiYuyueTypes : null,
                shujiYuyueDelete : null,
                insertTime : null,
                createTime : null,
            },
            dataList: [],
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'shuji_yuyue',//当前表表名,
                        sumColum : 'shuji_yuyue_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'shuji_yuyue_name',//分组字符串字段
                        // types : 'shuji_yuyue_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                if (this.searchForm.shujiYuyueTypes!= '' && this.searchForm.shujiYuyueTypes!= undefined) {
                    params['shujiYuyueTypes'] = this.searchForm.shujiYuyueTypes
                }
                ...
                //填充下拉框选项
                this.$http({
                    url: "dictionary/page?dicCode=shuji_yuyue_types&page=1&limit=100",
                    method: "get",
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.shujiYuyueTypesSelectSearch = data.data.list;
                    }
                });
            },
            //每页数
			...
			//提前还书
			refund(id){
				this.$confirm(`确定要提前还书吗?`, "提示", {
					confirmButtonText: "确定",
					cancelButtonText: "取消",
					type: "warning"
				}).then(() => {
					this.$http({
						url: "shujiYuyue/refund?id="+id,
						method: "post",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							this.$message({
			...
			deliver(id){
				let _this = this;
				_this.$confirm(`确定要到期吗?`, "提示", {
					confirmButtonText: "确定",
					cancelButtonText: "取消",
					type: "warning"
				}).then(() => {
					_this.$http({
						url: "shujiYuyue/deliver?id="+id,
						method: "post",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							_this.$message({
			...
			receiving(id){
				this.$confirm(`确定要还书吗?`, "提示", {
					...
					this.$http({
						url: "shujiYuyue/receiving?id="+id,
						method: "post",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							this.$message({
			...
			commentback(id){
				...
				_this.$confirm(`确定要评价吗?`, "提示", {
					...
					_this.$http({
						url: "shujiYuyue/commentback?id="+_this.commentbackId+"&commentbackText="+_this.commentbackContent+"&shujiCommentbackPingfenNumber="+_this.shujiCommentbackPingfenNumber,
						method: "post",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							_this.$message({
								...
								onClose: () => {
									...
									//情况评分
									_this.shujiCommentbackPingfenNumber = null
								}
							});
						} else {
							//清空操作数据id
